            ...
            wl_registry::Event::Global { name, interface, version } => {
                ...
                match interface.as_str() {
                    "ext_foreign_toplevel_list_v1" => {
                        let manager: ExtForeignToplevelListV1 = registry.bind(name, version, handle, ());
...
impl Dispatch<ExtForeignToplevelListV1, ()> for ToplevelManager {
    fn event(
        _state: &mut Self,
        _proxy: &ExtForeignToplevelListV1,
        _event: <ExtForeignToplevelListV1 as wayland_client::Proxy>::Event,
        _data: &(),
        _conn: &Connection,
    ...
    event_created_child!(ToplevelManager, ExtForeignToplevelListV1, [
        EVT_TOPLEVEL_OPCODE => (ExtForeignToplevelHandleV1, ())
    ]);
}

impl Dispatch<ExtForeignToplevelHandleV1, ()> for ToplevelManager {
    fn event(
        state: &mut Self,

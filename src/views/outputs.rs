...
mod tests {
    ...
    use super::MonitorArea;

    fn monitor(name: &str, x: i32, y: i32, width: u16, height: u16) -> Monitor {
        let workspace = WorkspaceBasic { id: 1, name: String::from("1") };
        Monitor {
            id: 0,
    ...
    #[test]
    fn a_layout_starting_at_the_origin_is_not_moved() {
        let area = MonitorArea::from(&vec![monitor("DP-1", 0, 0, 1920, 1080)]);

        assert_eq!((area.offset_x, area.offset_y), (0, 0));
    }

    #[test]
    fn a_layout_reaching_left_of_the_origin_is_pulled_right() {
        let monitors = vec![monitor("DP-1", -1920, -200, 1920, 1080), monitor("eDP-1", 0, 0, 1920, 1200)];

        let area = MonitorArea::from(&monitors);

        assert_eq!((area.offset_x, area.offset_y), (1920, 200));
    }

    // A laptop panel keeps the slot it was given next to monitors which are no longer plugged in.
    #[test]
    fn a_layout_starting_right_of_the_origin_is_pulled_back() {
        let area = MonitorArea::from(&vec![monitor("eDP-1", 4272, 460, 1600, 1000)]);

        assert_eq!((area.offset_x, area.offset_y), (-4272, -460));
    }

    #[test]
    fn the_offset_puts_the_first_monitor_in_the_corner() {
        let layouts = vec![
            vec![monitor("eDP-1", 0, 0, 1600, 1000)],
            vec![monitor("eDP-1", 4272, 460, 1600, 1000)],
            vec![monitor("DP-1", -1920, -200, 1920, 1080), monitor("eDP-1", 0, 0, 1600, 1000)],
            vec![monitor("DP-1", 1200, 96, 3072, 1728), monitor("eDP-1", 4272, 460, 1600, 1000)],
        ];

        for monitors in layouts {
            let area = MonitorArea::from(&monitors);

            assert_eq!((area.min_x + area.offset_x, area.min_y + area.offset_y), (0, 0), "layout {monitors:?}");
        }
    }

    #[test]
    fn the_offset_leaves_the_size_of_the_layout_alone() {
        let at_origin = MonitorArea::from(&vec![monitor("eDP-1", 0, 0, 1600, 1000)]);
        let far_right = MonitorArea::from(&vec![monitor("eDP-1", 4272, 460, 1600, 1000)]);

        assert_eq!((at_origin.width, at_origin.height), (far_right.width, far_right.height));
        assert_eq!(at_origin.aspect_ratio, far_right.aspect_ratio);
    }
}
